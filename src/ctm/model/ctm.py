...
from src.ctm.util.early_stopping import EarlyStopping
from src.ctm.network.decoding_network import DecoderNetwork
...
class CTM:
    def __init__(self, bow_size, contextual_size, inference_type="combined", n_components=10, model_type='prodLDA',
        ...
        if self.__class__.__name__ == "CTM":
            raise Exception("Can not use CTM, use ZeroShotTM or CombinedTM instead")
        ...
        if self.solver = "adam":
            ...
            raise Exception("For optimizer, use adam or sgd")
    ...
    def _train_epoch(self, loader):
        ...
        for batch_samples in loader: # Size of batch_samples: batch_size * vocab_size
            X_bow = 
            X_bow = 
            X_contextual = 
    ...
    def get_word_distribution_by_topic_id():
            pass
...
class ZeroShotTM(CTM):
    ...
    def __init__(self, **kwargs): # QUESTION: Why use **kwargs instead of *args?
        inference_type = "zeroshot"
        super()._init__(**kwargs, inference_type=inference_type) # QUESTION: Why parameters ZeroShotTM and self are not inside super() like super(ZeroShotTM, self)?
...
class CombinedTM(CTM):
    ...
    def __init__(self, **kwargs): # QUESTION: Why use **kwargs instead of *args?
        ...
        super().__init__(**kwargs, inference_type=inference_type) # QUESTION: Why parameters CombinedTM and self are not inside super() like super(CombinedTM, self)?
'''
